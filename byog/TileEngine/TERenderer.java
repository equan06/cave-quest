...
public class TERenderer {
    ...
    private int xOffset;
    private int yOffset;
    private int hudX; // specify bot left corner of HUD
    private int hudY;
    ...
    public void initialize(int w, int h, int xOff, int yOff) {
        ...
        this.xOffset = xOff;
        this.yOffset = yOff;
        this.hudX = 0; // bot left corner of HUD starts at [0, world height]
        this.hudY = h - 6;
    ...
    public void renderHUD(TETile[][] hud) {
        int numXTiles = hud.length;
        int numYTiles = hud[0].length;
        for (int x = 0; x < numXTiles; x += 1) {
            for (int y = 0; y < numYTiles; y += 1) {
                if (hud[x][y] == null) {
                    throw new IllegalArgumentException("Tile at position x=" + x + ", y=" + y
